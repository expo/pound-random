...
class Api {
  ...
  async getSessionFromLoginCodeAsync(mobileNumber, code) {
    return login.checkCodeForMobileNumberAsync(mobileNumber, code);
  }
  async loginOrSignupAsync(mobileNumber) {
    return await login.loginWithMobileNumberAsync(mobileNumber);
  }
  ...
  async newPostAsync({ content, url, replyTo, contentImage }) {
    let userId = this.context.userId;
    let postId = await post.newPostAsync(userId, content, url, replyTo, contentImage);
    return postId;
  }
  ...
  async threadAsync(postId) {
    return await post.getRepliesToPost(postId);
  }
  ...
  async sortOfLoginAsync(rawUsername) {
    ...
    if (!userId) {
      throw clientError('USERNAME_NOT_FOUND', "No user with the username '" + rawUsername + "'", {
        rawUsername,
        normalizedUsername,
      });
    }
  ...
  async createPostAsync({ content, replyTo }) {
    let userId = this.context.userId;
    if (!userId) {
      throw clientError('LOGIN_REQUIRED', 'Login required to post');
    }
    return await post.newPostAsync(userId, content, replyTo, new Date());
  }

  async deletePostAsync(postId, userId) {
    // For now, you can only delete your own posts
    return await post.deletePostAsync(postId, userId);
  }
